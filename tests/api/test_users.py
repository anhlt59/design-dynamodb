...
from app.domain.adapters.unit_of_works import UserUnitOfWork
from app.domain.adapters.use_cases import UserUseCase

user_use_case = UserUseCase(UserUnitOfWork())
...
def test_register_login_user(test_client):
    ...
    user = user_use_case.login("test01@gmail.com", "12345678")
...
def test_update_user(test_client, dummy_user):
    ...
    assert rv.status_code == 200
    user = user_use_case.get_by_id(dummy_user.id)
