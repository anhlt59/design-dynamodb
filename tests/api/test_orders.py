...
def test_create_order(app_context, test_client, dummy_user, dummy_product):
    data = {
    ...
    app_context.order_use_case.delete(dummy_user.id, rv.json()["id"])
    # out of stock
...
def test_cancel_order(app_context, test_client, dummy_order):
    rv = test_client.put(f"/api/v1/users/{dummy_order.userId}/orders/{dummy_order.id}")
    assert rv.status_code == 200

    order = app_context.order_use_case.get(dummy_order.userId, dummy_order.id)
