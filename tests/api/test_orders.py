...
def test_get_order(test_client, dummy_order):
    rv = test_client.get(f"/api/v1/users/{dummy_order.userId}/orders/{dummy_order.id}", headers=HEADERS)
    assert rv.status_code == 200
    assert rv.json["id"] == dummy_order.id


def test_list_order(test_client, dummy_order):
    rv = test_client.get(f"/api/v1/users/{dummy_order.userId}/orders", headers=HEADERS)
    assert rv.status_code == 200
...
def test_cancel_order(test_client, dummy_order):
    rv = test_client.put(f"/api/v1/users/{dummy_order.userId}/orders/{dummy_order.id}", headers=HEADERS)
    assert rv.status_code == 200
